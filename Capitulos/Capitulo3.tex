...
Este capítulo apresenta o conjunto de visualizações proposto como extensão à ferramenta \textit{\perfMinerName}, a qual foi discutida na seção \ref{sec:perfminer}. A seção \ref{sec:visao-geral-performance-qa-evolution} mostra uma visão geral sobre a ferramenta proposta. As seções \ref{sec:visualizacao1} e \ref{sec:visualizacao2} descrevem as visualizações da sumarização de cenários e grafo de chamadas, respectivamente, destacando suas características, propriedades visuais e funcionamento. Por fim, são reportadas considerações finais sobre o capítulo na seção \ref{sec:consideracoes-cap3}.

\section{Visão Geral do \toolName} \label{sec:visao-geral-performance-qa-evolution}

A extensão ao \textit{\perfMinerName} foi implementada na forma de uma aplicação web, intitulada de \textit{\toolName}. O processamento do \textit{\perfMinerName} em suas fases 1, 2 e 3 (conforme explicado na seção \ref{sec:perfminer}) não foi alterado, continuando \textit{standalone}. A escolha por esse tipo de aplicação se deu pelo fato de sua execução ocorrer em um ambiente distribuído, onde cada parte que compõe a aplicação está localizada em locais diferentes. Por exemplo, a interface com o usuário reside em sua estação de trabalho, ao passo que o servidor e o banco de dados estão localizados em outro computador.
...
\begin{figure}[!htb]
   \centering
   \frame{\includegraphics[scale=0.52]{Imagens/perfminer_fase_4_atual.png}}
...
Quando o servidor (\textit{\toolName}) recebe essa requisição, é verificado que todos os arquivos foram enviados e que não existe uma análise já realizada para o sistema e versões indicadas, o usuário recebe a resposta (passo 2) de que o processamento requisitado está em andamento (\texttt{PENDING}). A requisição assíncrona evita que o usuário necessite esperar o processamento terminar para continuar utilizando a ferramenta. A partir do momento em que a resposta é enviada, o usuário pode acompanhar o processamento da sua requisição na página inicial da ferramenta (Figura \ref{fig:pagina-inicial}).

Dando continuidade ao processo requisitado, o sistema recupera os arquivos de \textit{backup} dos modelos de análise dinâmica enviados pelo usuário e os restaura no banco de dados DAM \abrv[DAM -- \textit{Dynamic Analysis Model}]{(\textit{Dynamic Analysis Model}),}como indica o passo 3. Após isso, inicia-se o processamento em lote (passo 4). De posse dos dados informados, é realizado um processamento para determinar todos os dados que dão suporte às visualizações oferecidas pela ferramenta. Após isso, no passo 5, os dados resultantes são salvos em um banco de dados (identificado por \abrv[QAV -- \textit{Quality Attribute Visualization}]{QAV}– \textit{Quality Attribute Visualization}).

Detalhadamente, no processamento em lote, iniciado no passo 4, os arquivos de saída do \textit{\perfMinerName} contendo informações sobre os cenários e métodos com desvio de desempenho são lidos e os modelos de análise dinâmica de ambas as versões são recuperados do banco de dados DAM. A partir de então, os dados para cada visualização começam a ser processados. Os passos 4 e 5 da Figura \ref{fig:funcionamento-geral-nova-analise} são detalhados no diagrama de atividades da Figura \ref{fig:nova-analise-passos-4-5}.
...
\begin{figure}[!htb]
   ...
   \textsf{\caption[Passos 4 e 5 da realização de uma nova análise.]{Passos 4 e 5 da realização de uma nova análise.\label{fig:nova-analise-passos-4-5}}}
\end{figure}
...
\noindent \textbf{QAV - \textit{Quality Attribute Visualization}}

A ferramenta faz uso de outro banco de dados além do DAM: o QAV. Essa base de dados é responsável por armazenar os dados que darão suporte às visualizações. No nível orientado a objetos, a Figura \ref{fig:archvis-class-diagram} a seguir mostra o diagrama de classes para esse modelo.
...
Os cenários de destaque nessa visualização são dois, em particular: (i) o que possuiu maior porcentagem de desvio de desempenho; e (ii) o que possuiu maior tempo de execução, indicados, respectivamente, pela largura e altura de uma fatia. Dentre esses dois cenários, a principal indicação da visualização é o que possuiu maior porcentagem desvio de desempenho em relação a versão anterior: o cenário com maior largura no gráfico. Assim sendo, o cenário \texttt{B} requer maior atenção por parte dos desenvolvedores do que o cenário \texttt{A}.
...
\begin{figure}[!htb]
   \centering
   \includegraphics[scale=0.62]{Imagens/funcionamento_geral_visualizacoes_atual.png}
...
Nesse sentido, em uma chamada de métodos é evidente a relação entre eles, uma vez que os objetos, em um sistema orientado a objetos, trocam mensagens entre si através da invocação dos métodos. O grafo é exibido utilizando o \textit{layout} em árvore, onde os nós filhos estão dispostos abaixo dos nós ancestrais, e a direção dos nós é \textit{top-down}. A Figura \ref{fig:pagina-grafo-chamadas-exemplo} mostra um trecho da página web que apresenta o grafo de chamadas. Essa trecho é dividido em três seções: Sumário, Histórico e o próprio Grafo de Chamadas, as quais são descritas mais detalhadamente a seguir. Por limitações de espaço neste documento, a seção do Grafo de Chamadas foi suprimido na Figura \ref{fig:pagina-grafo-chamadas-exemplo}, no entanto um exemplo desse grafo pode ser visto na Figura \ref{fig:grafo-chamadas-exemplo}.

%Nesta seção serão detalhadas as suas propriedades visuais, o seu funcionamento básico, além de descrever a seção de Sumário e Histórico desta visualização.

\begin{figure}[htb]
   \centering
   \frame{\includegraphics[scale=0.41]{Imagens/pagina_grafo_chamadas_exemplo.png}}
   \textsf{\caption[Trecho de exemplo da página web do grafo de chamadas.]{Trecho de exemplo da página web do grafo de chamadas.\label{fig:pagina-grafo-chamadas-exemplo}}}
\end{figure}
...
O primeiro e mais básico tipo de nó é o que representa um método que não teve desvios de desempenho para o cenário e versões analisadas, apresentado na Figura \ref{fig:no-sem-desvio}. As características desse nó são:
\begin{itemize}
...
\begin{itemize}
   \item \textit{Cor}: os nós que degradaram o desempenho são mostrados na cor mostarda;
...
\begin{itemize}
   ...
   \item \textit{Setas}: as setas indicativas de variação de desempenho para nós de otimização são apontadas para cima, de cor verde. No exemplo, a otimização se deu em mais de 75\% do desempenho em relação à versão anterior.
...
\begin{table}[!htb]
   ...
   Degradado     & Mostarda & Total, próprio e desvio & Vermelhas, para baixo \\ \hline
...
\begin{figure}[htb]
   \centering
   \frame{\includegraphics[scale=0.55]{Imagens/grafo_chamadas_legenda.png}}
...
\begin{figure}[htb]
   \centering
   \frame{\includegraphics[scale=0.49]{Imagens/grafo_chamadas_detalhes_no.png}}
