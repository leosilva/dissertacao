...
\citeauthor{Seriai2014} realizaram um estudo de mapeamento sistemático de métodos de validação em visualização de software. Os autores definiram seis propriedades de classificação desses métodos:
\begin{enumerate}[(i)]
...
A ferramenta \textit{\toolName}, proposta nesta dissertação, foi desenvolvida como uma extensão da ferramenta \textit{\perfMinerName}, a qual provê a avaliação de cenários de execução do sistema no que diz respeito a desvios de desempenho, de forma a minimizar a erosão de tal atributo de qualidade \cite{Pinto2015}.
